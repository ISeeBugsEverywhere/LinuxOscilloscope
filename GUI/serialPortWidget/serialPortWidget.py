...
class serialPortWidget(QtWidgets.QWidget):
    """
    class for the widget, intended to use for a configuration of a rs232 port
    """
    ...
    def return_serial_dict(self):
        ...
        defined: baudrate, bytesize, parity, stopbits
        :return:
        """
        params = {}
        params['baudrate'] = int(self.ui.baudRateBox.currentText())
        params['bytesize'] = int(self.ui.dataBitsBox.currentText())
        p = int(self.ui.parityBox.currentText())
        parity_val = self._parity_serial[p]
        params['parity'] = parity_val
        stop_bts = int(self.ui.stopBitsBox.currentText())
        stp_bts = self._stopbits_serial[stop_bts]
        params['stopbits'] = stp_bts
        return params
        pass